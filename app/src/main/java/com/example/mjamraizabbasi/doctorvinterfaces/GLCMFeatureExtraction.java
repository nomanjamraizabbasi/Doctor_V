...
public class GLCMFeatureExtraction {
    ...
    public GLCMFeatureExtraction(Context context, Mat image1, int grayLevel){
        this.context = context;
        this.image = image1;
        this.grayLevel = grayLevel;
//        for(int i = 0;i<this.image.rows();i++){
//            for(int j = 0;j<this.image.cols();j++){
//                Log.d("OUTPUT",Double.toString(image.get(i,j)[0])+",,,,,"+i+",,,,,"+j);
//            }
//        }
...
//        thread.start();
        ...
        grayLeveledMatrix = new int[this.image.rows()][this.image.cols()];
    }
    ...
    private void createGrayLeveledMatrix() {
        for (int i = 0; i < this.image.rows(); i++) {
            for (int j = 0; j < this.image.cols(); j++) {
               //Log.d("Output",Double.toString(image.get(i,j)[0])+",,,,"+i+",,,,,"+j+",,,,,,"+image.rows());
                int gr = (int)(image.get(i, j)[0]);
